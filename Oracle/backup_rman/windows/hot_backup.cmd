...
@set TARGET_TNS='/'
@set POLICY_NAME=bung76orawin_PPI
@set SCHED_NAME=Default-Application-Backup-diario-PPI
...
echo BACKUP %BACKUP_TYPE% SKIP INACCESSIBLE FILESPERSET 30 FORMAT 'bkp_dbf_s%%s_p%%p_t%%t' DATABASE;
...
echo send 'NB_ORA_POLICY=%POLICY_NAME%';
echo send 'NB_ORA_SCHED=%SCHED_NAME%';
echo BACKUP FILESPERSET 20 FORMAT 'bkp_arch-s%%s-p%%p_t%%t' ARCHIVELOG ALL delete input;
echo RELEASE CHANNEL ch00;
echo ALLOCATE CHANNEL ch00 TYPE 'SBT_TAPE';
echo send 'NB_ORA_POLICY=%POLICY_NAME%';
echo send 'NB_ORA_SCHED=%SCHED_NAME%';
echo BACKUP FORMAT 'bkp_cntrl-s%%s-p%%p_t%%t' CURRENT CONTROLFILE;
echo RELEASE CHANNEL ch00;
echo }
