...
begin
	...
	for rc1 in c1 loop                 
		...
		if qtde = 0 then 
			dbms_output.put_line('A tabela '||rc1.table_name||' não existe na &&base_remota');
		else
			for rc2 in c2(rc1.owner, rc1.table_name) loop
				select count(1) into qtde from dba_tab_columns_temp where owner = upper('&&owner_destino')
				...
				if qtde = 0 then
					dbms_output.put_line('O campo '||rc2.column_name||' da tabela '||rc1.table_name||
															 ' não existe na base &&base_remota');
				else
					v_DATA_TYPE       := 0;
					v_DATA_LENGTH     := 0;
					v_DATA_PRECISION  := 0;
					...
					if rc2.data_type <> v_data_type then
						...
						if rc2.data_type = 'NUMBER' then
							if (RC2.DATA_PRECISION <> v_DATA_PRECISION or rc2.DATA_SCALE <> v_DATA_SCALE) then
								dbms_output.put_line ('A precisão do campo '||rc2.column_name||' da tabela '||rc1.table_name||' é diferente');
							end if;
						elsif (rc2.data_type = 'VARCHAR2' OR rc2.data_type='VARCHAR' or rc2.data_type='CHAR' ) then
						  if (rc2.DATA_LENGTH <> v_DATA_LENGTH) then 
							  dbms_output.put_line ('O tamanho do campo '||rc2.column_name||' da tabela '||rc1.table_name||' é '||rc2.DATA_LENGTH||' na origem e '||v_DATA_LENGTH||' na &&base_remota'); 
							end if;
						end if;
						v_DATA_DEFAULT2 := rc2.DATA_DEFAULT; 
						--verificacao de valor default
						if v_DATA_DEFAULT <> v_DATA_DEFAULT2 then
							dbms_output.put_line ('O valor DEFAULT do campo '||rc2.column_name||' da tabela '||rc1.table_name||' é '||v_DATA_DEFAULT2||' na origem e '||v_DATA_DEFAULT||' na &&base_remota');
						end if;
					end if;
			...
			-- verificacao de indices
				if qtde = 0 then
				  ...
				  for rc_col in (select table_owner, table_name, column_name, column_position from dba_ind_columns
				  	...
				  	if qtde = 0 then
				  		dbms_output.put_line ('O índice '||rc3.index_name||' para a tabela '
							||rc1.table_name||' não existe ou está diferente na &&base_remota');
							exit;
				  	end if;
				  	qtde2 := qtde2 + 1;
					end loop;
					...
					*/
					...
					if qtde > 0 then 
						...
						dbms_output.put_line ('O índice '||rc3.index_name||' da tabela '||rc1.table_name
				  	||' está com nome diferente na &&base_remota');	
			  	...
			  	select uniqueness into v_UNIQUENESS from dba_indexes_temp 
					...
					if (rc3.UNIQUENESS <> v_UNIQUENESS) then
						dbms_output.put_line ('O índice '||rc3.index_name||' da tabela '
						||rc1.table_name||' não é '||rc3.UNIQUENESS);
					end if;
					qtde_campo_o := 0;
					qtde_campo_d := 0;
					...
					if qtde_campo_o <> qtde_campo_d then
						...
						for rc4 in c4(rc1.owner, rc1.table_name, rc3.index_name) loop
				  		...
				  		if rc4.column_position <> v_posicao then
				  			if v_posicao = 0 then
				  				dbms_output.put_line ('A coluna '||rc4.column_name||' do índice '
									||rc3.index_name||' não faz parte do indíce na &&base_remota');	
				  			else					
									dbms_output.put_line ('A coluna '||rc4.column_name||' do índice '
									||rc3.index_name||' está na posicao '||v_posicao||' sendo que a origem é:'
									||rc4.column_position);
								end if;
				  		end if;
						end loop;	
				...
				--verificacao de constraint com nome diferente
					if rc5.constraint_type='C' then
					  ...
					  qtde := 0;
						...
						if qtde = 0 then
							dbms_output.put_line('A constraint '||rc5.constraint_name||' da tabela '
						  	||rc1.table_name||' não existe na &&base_remota');
						end if;
				  				...
				  				if rc6.position <> v_posicao then
				  					if v_posicao = 0 then
				  						dbms_output.put_line ('A coluna '||rc6.column_name||' da constraint '
											||rc5.constraint_name||' não faz parte da constraint na &&base_remota');
				  					else				
											dbms_output.put_line ('A coluna '||rc6.column_name||' da constraint '
											||rc5.constraint_name||' está na posicao '||v_posicao||' sendo que a origem é:'
											||rc6.position);
										end if;
				  				end if;
								end loop;	
