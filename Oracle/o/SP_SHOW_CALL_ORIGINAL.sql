CREATE OR REPLACE
PROCEDURE sp_show_call_original (t IN VARCHAR2, c IN NUMBER) IS
--
...
--
  ...
  cursor c_defcall (c NUMBER, t VARCHAR2) is
     select callno, deferred_tran_id, schemaname, packagename,
procname,argcount
       from defcall
...
package_name
  ...
  cursor pk_cursor (schema VARCHAR2, t_name VARCHAR2, col_name
VARCHAR2)
is
...
||c1rec.schemaname||'.'||c1rec.packagename||
'.'||c1rec.procname);
...
' || '----------------------');
    ...
    open c_arg_name (c1rec.schemaname, c1rec.procname,
c1rec.packagename, c1rec.argcount);
    ...
    WHILE argno <= c1rec.argcount LOOP
      ...
      table_name := substr(c1rec.packagename, 1,
instr(c1rec.packagename, '$') - 1);
...
' '||nvl(varchar2_val,'(NULL)'));
        ...
        typdsc := 'NVARCHAR2';
        nvarchar2_val := dbms_defer_query.get_nvarchar2_arg(callno,
argno,tranid);
...
' '||nvl(char_val,'(NULL)')||'|');
        ...
        typdsc := 'NCHAR';
        nchar_val := dbms_defer_query.get_nchar_arg(callno, argno,
tranid);
...
' '||nvl(translate(nchar_val using char_cs),'(NULL)')||'|');
        ...
        typdsc := 'BLOB';
        varchar2_val :=
dbms_lob.substr(dbms_defer_query.get_blob_arg(callno,
argno, tranid));
...
' '||nvl(varchar2_val,'(NULL)'));
        ...
        typdsc := 'CLOB';
        varchar2_val :=
dbms_lob.substr(dbms_defer_query.get_clob_arg(callno,
argno, tranid));
...
' '||nvl(varchar2_val,'(NULL)'));
        ...
        typdsc := 'NCLOB';
        nvarchar2_val :=
dbms_lob.substr(dbms_defer_query.get_nclob_arg(callno,
argno, tranid));
