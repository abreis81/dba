rem 
rem nome        : lst_role_sys.sql
rem objetivo    : Roles x privilegios de sistema
...
--
define p_sql       = lst_role_sys
define p_titulo    = 'Roles x privilegios de Sistema'
...
rem
rem	Query 1 - Privilegios de Sistema
rem
  select grantee
        ,privilege
        ,admin_option
    from dba_sys_privs a
        ,dba_roles b
   where role like upper(decode('&&p_role','','%','&&p_role'))
